...
void Parser::extractPrefix(const std::string &line, IRCMessage &msg, size_t &pos)
{
    if(line[pos] == ':')
    {
 ...
 void Parser::extractCommand(const std::string &line, IRCMessage &msg, size_t &pos)
{
 ...
 void Parser::extractParam(const std::string &line, IRCMessage &msg, size_t &pos)
{
    while(pos < line.size())
    {
        if(line[pos] == ':')
